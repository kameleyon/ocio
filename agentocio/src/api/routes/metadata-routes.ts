...
import express, { Router, Request, Response } from 'express'; // Import Request, Response
import { z } from 'zod';
...
export default function metadataRoutes(storage: Storage): Router {
   ...
   * GET /api/metadata
   */
  router.get('/', validation.validate(metadataQuerySchema, 'query'), async (req: Request, res: Response) => { // Add types
    try {
   ...
   * GET /api/metadata/:id
   */
  router.get('/:id', async (req: Request, res: Response) => { // Add types
    try {
      const { id } = req.params;
   ...
   * POST /api/metadata
   */
  router.post('/', validation.validate(metadataSchema), async (req: Request, res: Response) => { // Add types
    try {
   ...
   * PUT /api/metadata/:id
   */
  router.put('/:id', validation.validate(metadataSchema), async (req: Request, res: Response) => { // Add types
    try {
      const { id } = req.params;
   ...
   */
  router.delete('/:id', async (req: Request, res: Response) => { // Add types
    try {
      const { id } = req.params;
