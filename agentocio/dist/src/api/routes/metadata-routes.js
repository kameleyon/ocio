 ...
 */
import { Router } from 'express'; // Import Request, Response
import { z } from 'zod';
 ...
 */
export default function metadataRoutes(storage) {
    ...
    router.get('/', validation.validate(metadataQuerySchema, 'query'), async (req, res) => {
        try {
            ...
            const filter = isAdmin ? {} : { userId: req.user?.id || 'anonymous' };
            // Get items with type casting
            const items = await storage.find(metadataCollection, filter);
            // Apply pagination
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const item = await storage.findById('metadata', id);
            if (!item) {
    ...
    router.post('/', validation.validate(metadataSchema), async (req, res) => {
        try {
            ...
            const item = await storage.create('metadata', metadata);
            res.status(201).json({
    ...
    router.put('/:id', validation.validate(metadataSchema), async (req, res) => {
        try {
            ...
            const existingItem = await storage.findById('metadata', id);
            if (!existingItem) {
                return res.status(404).json({
                    error: {
                        message: `Metadata with ID '${id}' not found`,
            ...
            // Update the metadata
            const updatedItem = await storage.updateById('metadata', id, {
                ...req.body,
